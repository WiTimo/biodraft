...
# 6) Plot each loop segment separately to avoid drawing connecting spikes
fig = plt.figure(figsize=(8, 6))
ax  = fig.add_subplot(projection="3d")
for i in range(len(breaks) - 1):
    start, end = breaks[i], breaks[i+1]
    seg_thetas = thetas[start:end]
    # keep segments spanning at least half wrap (π)
    if np.ptp(seg_thetas) >= np.pi:
