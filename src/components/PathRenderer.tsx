...
type Props = {
    ...
    mode: Mode;
    drawStartPoint: BezierPoint | null;
        ...
        <>
            ...
            {currentPoints.length > 0 && previewPoint && (
                <Shape
                    listening={false}
                    sceneFunc={(ctx, shape) => {
                        const p0 = drawStartPoint ?? currentPoints[currentPoints.length - 1];
            ...
            {[...currentPoints, ...paths.flatMap((p) => p.points), ...(previewPoint ? [previewPoint] : [])].filter((point, index, self) => self.findIndex(p => p.id === point.id) === index).map((point, idx) => (
                <Fragment key={`${point.id}-${idx}`}>
                    ...
                    />
                    {(point.id === selectedPointId || point === previewPoint) && (
                        <>
                            {point.handleLeft && (
                                <>
                                    <Line
                                    ...
                                    <Circle
                                        ...
                                        draggable={mode === "SELECT"}
                                        onDragMove={(e) => {
                                            onHandleDragMove(point.id, "left", e.target.position(), e.evt.altKey);
                                        }}
